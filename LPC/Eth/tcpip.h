...
#define MAX_ETH_TX_DATA_SIZE 60                    // enough to echo 32 byte via ICMP

#define DEFAULT_TTL          64                  // Time To Live sent with packets
//#define DEFAULT_TTL          128                  // Time To Live sent with packets
...
unsigned short RemoteIP[] =                    // "MYIP1.MYIP2.MYIP3.MYIP4"
{
  REMIP_1 + (REMIP_2 << 8),                        // use 'unsigned int' to
  REMIP_3 + (REMIP_4 << 8)                         // achieve word alignment
};
...
extern char _TxFrame1[(ETH_HEADER_SIZE + IP_HEADER_SIZE + TCP_HEADER_SIZE + MAX_TCP_TX_DATA_SIZE)];
extern char _TxFrame2[(ETH_HEADER_SIZE + MAX_ETH_TX_DATA_SIZE)];
extern char _RxTCPBuffer[MAX_TCP_RX_DATA_SIZE]; // space for incoming TCP-data
...
#define OP_IP_ADDR_RESOLVED			   0x20
